...
// 你的 Firebase 設定物件 (從 Firebase Console 複製貼上)
const firebaseConfig = {
    apiKey: "AIzaSyCqF2NtEbv_9ldLoOWOd5_MUIZUnIqNiuY",
    authDomain: "better-vegetable-catcher.firebaseapp.com",
    projectId: "better-vegetable-catcher",
    storageBucket: "better-vegetable-catcher.firebasestorage.app",
    messagingSenderId: "657168292147",
    appId: "1:657168292147:web:f90d3d6fb3923897c90646",
    measurementId: "G-YMQCKQL637"
  };
...
console.log("Firebase 和 FCM 已初始化");

// --- 接下來我們將處理通知權限和取得 Token ---
// (這部分在下一步)

// 處理在前台時接收到的通知
onMessage(messaging, (payload) => {
  console.log('收到前台訊息:', payload);
  // 你可以在這裡顯示一個自訂的通知或更新 UI
  // 例如：使用 Notification API 顯示通知
  const notificationTitle = payload.notification.title;
  const notificationOptions = {
    body: payload.notification.body,
    icon: '/firebase-logo.png' // 你可以替換成你的網站圖示
  };

  if (Notification.permission === "granted") {
      new Notification(notificationTitle, notificationOptions);
  }
  // 也可以不做任何事，因為背景 Service Worker 會處理標準的推播通知
});

// 在使用者執行某個動作後，或在應用程式初始化時呼叫這個函式
      ...
      if (permission === 'granted') {
        ...
        getToken(messaging, { vapidKey: 'YOUR_VAPID_KEY' }).then((currentToken) => {
          if (currentToken) {
            ...
            // TODO: 將這個 currentToken 發送到你的伺服器或儲存到資料庫
            // 例如：saveTokenToDatabase(currentToken);
            console.log('請將此 Token 儲存到後端或資料庫，以便透過 FCM 發送訊息給此裝置');
          } else {
            // 使用者尚未授與通知權限，或發生錯誤
            console.log('沒有可用的 FCM 註冊 Token。可能尚未授與權限或阻擋了通知。');
            // 根據情況顯示 UI 提示使用者開啟通知
          }
        }).catch((err) => {
          console.error('取得 FCM 註冊 Token 時發生錯誤:', err);
        });
  
      } else {
        console.log('通知權限被拒絕');
        // 根據情況顯示 UI 提示使用者已拒絕通知
      }
    });
  }
  
  // 範例：可以在頁面載入完成後呼叫這個函式 (或者在使用者點擊按鈕後)
  // window.onload = requestNotificationPermission; // 這可能太早，建議在使用者互動後觸發
